...
namespace Exadel.HEH.Backend.BusinessLogic
{
    ...
    public class Result<T> : Result
        where T : class
    {
        protected Result(bool success, bool error, string message, T data)
        ...
        public static Result<T> Quite<T>(string message)
            where T : class
        {
            return new Result<T>(false, false, message, null);
        }

        public static Result<T> Quite<T>(string message, T data)
            where T : class
        {
